...
class MessageHeaderValidator
{
	...
	private void validateFrom(MessageHeader messageHeader)
	{
		...
		if ((cpaManager.getPartyInfo(messageHeader.getCPAId(),messageHeader.getFrom().getPartyId())).isPresent())
			throw new EbMSValidationException(
				    EbMSMessageUtils.createError("//Header/MessageHeader/From/PartyId",EbMSErrorCode.INCONSISTENT,VALUE_NOT_FOUND));
	}
	...
	private void validateTo(MessageHeader messageHeader)
	{
		...
		if ((cpaManager.getPartyInfo(messageHeader.getCPAId(),messageHeader.getTo().getPartyId())).isPresent())
			throw new EbMSValidationException(
				    EbMSMessageUtils.createError("//Header/MessageHeader/To/PartyId",EbMSErrorCode.INCONSISTENT,VALUE_NOT_FOUND));
	}
	...
	private void validateService(MessageHeader messageHeader, String service)
	{
		if (!cpaManager.canSend(messageHeader.getCPAId(),messageHeader.getFrom().getPartyId(),messageHeader.getFrom().getRole(),service,messageHeader.getAction()))
			throw new EbMSValidationException(
					EbMSMessageUtils.createError("//Header/MessageHeader/Action",EbMSErrorCode.VALUE_NOT_RECOGNIZED,VALUE_NOT_FOUND));
	...
	private void validateMessageData(Instant timestamp, MessageHeader messageHeader)
	{
		if (!existsRefToMessageId(messageHeader.getMessageData().getRefToMessageId()))
			throw new EbMSValidationException(
					EbMSMessageUtils.createError("//Header/MessageHeader/MessageData/RefToMessageId",EbMSErrorCode.VALUE_NOT_RECOGNIZED,VALUE_NOT_FOUND));
